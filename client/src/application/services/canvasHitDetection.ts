...
import { pxToMm } from './canvasUtils';
import { PT_TO_MM } from '../../../../shared/src/constants';
 ...
 */
export function calculateLineHitbox(element: TitlePageElement, padding: number = HITBOX_PADDING): Hitbox {
...
export function isPointInElement(
  ...
  return x >= hitbox.minX && x <= hitbox.maxX && y >= hitbox.minY && y <= hitbox.maxY;
}
...
export function findElementAtPoint(
  ...
  ctx: CanvasRenderingContext2D,
  getDistanceToElement: (x: number, y: number, el: TitlePageElement, ctx: CanvasRenderingContext2D) => number | null
