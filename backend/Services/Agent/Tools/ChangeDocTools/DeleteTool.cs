...
public class DeleteTool : ITool
{
    ...
    public DeleteTool(IDocumentService documentService, ILogger<DeleteTool> logger)
    {
    ...
    public Dictionary<string, object> GetParametersSchema()
    {
        return new Dictionary<string, object>
        {
            ...
            {
                ["id"] = new Dictionary<string, object> { ["type"] = "integer", ["description"] = "1-based номер первой строки" },
                ["id_end"] = new Dictionary<string, object> { ["type"] = "integer", ["description"] = "1-based номер последней строки (опционально)" }
            },
            ["required"] = new[] { "id" }
        };
    }
    ...
    public async Task<string> ExecuteAsync(Dictionary<string, object> arguments, CancellationToken cancellationToken = default)
    {
        ...
        int? idEnd = arguments.ContainsKey("id_end") ? GetIntValueFlexible(arguments, "id_end") : null;

        if (id <= 0) return "Ошибка: id должен быть >= 1";
        ...
        var startIndex = id - 1;
        if (startIndex >= lines.Count) return $"Ошибка: id={id} вне диапазона";
        ...
        if (idEnd.HasValue)
        {
            ...
            startIndex = minId - 1;
            endIndex = Math.Min(maxId - 1, lines.Count - 1);
        }
        ...
        return deleteCount == 1 ? $"delete: удалена строка {startIndex + 1}" : $"delete: удалены строки {startIndex + 1}-{endIndex + 1}";
    }
