...
export function useDocumentChats() {
	...
	const loadChats = async (documentId: string) => {
		...
		try {
			const allChats = await ChatAPI.getChats('document', documentId, true);
	...
	const loadChatById = async (chatId: string, options?: { silent?: boolean }) => {
		...
		try {
			...
			if (currentDocumentId.value && chat.documentId !== currentDocumentId.value) {
				return;
			}
	...
	const createChat = async (documentId: string, title?: string) => {
		...
		currentDocumentId.value = documentId;
		try {
			const newChat = await ChatAPI.createChat({ scope: 'document', documentId, title });
			loadChatsRequestId++;
